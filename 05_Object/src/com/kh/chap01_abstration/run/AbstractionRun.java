...
public class AbstractionRun {
	 ...
	 * 지향>> 좋음(추구한다.)
	 * 
	 * 2 객체란: 현실세계에 독립적으로 존재(목적이 있고 가치가 있고 의미가 있는)하는 모든것들을 의미
	 * (유형,무형,개념,)
	 * 현실세계에서는 객체들간의 상호작용으로 돌아감(내가 물을 먹는다)
	 * 
	 * 3 객체지향 프로그래밍 : 현실세계의 객체들간의 상호작용
	 * 					  프로그레밍을 통해 가상세계로 구현하는 것 (내가 쇼핑몰에서 로그인 한다)
	 * 
	 * 4 구현하고자 하는 프로그램 객체들을 만들기 위해서 (즉 생성하기위해)
	 *    클래스라는 틀을 먼저 만들어야됨 (클래스란? 각 객체들의 속성(정보)들을 담아낼 그릇같은 존재)
	 *    이름,나이,주소,휴대폰 번호 등등 담을 수 있는 그릇
	 *    >>어떻게 만드는 걸까? >> 추상화+캡슐화 접목해서 만들어야함
	 *    
	 *
	 * 5 추상화 과정
	 *  1 내가 구현하고자 하는 프로그램에서 필요한 객체들을 생각해 볼것
	 *  2 그 객체들이 가지는 공통적인 속성 기능들을 모두 추출 >>이름,나이,주소,휴대폰번호,혈액형
	 *  3 추출한 것들을 가지고 내가 구현하고자 했던 프로그램"실질적인 목적"에 맞춰 불필요한 속성 기능들을 제거할것
	 *  4 그렇게 최종적으로 추려진 속성들을 가지고 어떤 자료형으로 어떤 변수명으로 사용할 건지 생각하는것을 추상화라고 함
	 *  
	 *  Ex 학생 관리 프로그램
	 ...
	 *  	  학생 성적을 관리하는 프로그램이라고한다면 : 이름 학년 반 국어점수 수학점수 영어점수
	 *   	  학생 건강을 관리하는 프로그램이라고 한다면 이름 학년 반 키 몸무게 달리기속도 먹는다 잠잔다
	 *   	
	 *   
