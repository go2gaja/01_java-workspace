...
public class ExceptionRun {
	...
	public static void main(String[] args) {
		 ...
		 * -컴파일 에러 : 소스코드 문법상 오류>>빨간줄로 애초에 오류를 알려줌 (개발자의 실수)
		 * -런타임 에러 : 코드상으로 문제가 없는데 프로그램 실행중에 발생하는 에러(사용자의 실수 일 수도 있고)
		 * 
		 * -논리 에러   : 문법적으로 문제없고 실행했을때도 굳이 문제는 없지만 프로그램 의도상 맞지 않는것
		 * 시스템 에러를 제외한 컴파일 에러 ,런타임 에러, 논리 에러와 같은 비교적 덜 심각한 것들
		 * 이런것들을 "예외"라고 한다!!!>exception
		 * 
		 * 이런 "예외"가 "발생"했을 경우에 대해서"처리"하는 방법을"예외처리"라고 함
		 * 
		 * 예외처리 목적 예외처리를 하지 않고 그대로 예외가 발생되는 경우 프로그램이 비정상적으로 종료될 수 있음
		 * 
		 * 예외처리 방법
