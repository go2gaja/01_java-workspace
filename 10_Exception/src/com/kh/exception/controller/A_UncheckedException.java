...
public class A_UncheckedException {
	 ...
	 * -ClassCastException : 허용할 수 없는 형 변환이 진행될 때 발생되는 예외
	 * -NegativeArraySizeException : 배열 할당 과정에서 배열의 크기를 음수로 지정하는경우 발생되는 예외
	 * 
	 * >>RuntimeException 관련된 예외는 충분히 예측가능한 상황이 있음
	 * 	 아싸리 애초에 예외 발생이 안되게끔 if 조건 처리 가능
	 * 
	 */
	...
	public void method1() {
		...
		//애초에 예외 자체가 발생이 안되게끔 if 문으로 조건 검사
		/*
		int result = 0;
		if (num2 !=0) {
			result = num1/num2 ;
		...
		*/
		...
		//예외 처리 : 예외가 "발생했을때" 실행할 내용을 미리 작성해놓는 방법
		// 표현법
		// try catch 방법
		// try{
		// 예외가 발생될 수도 있는 구문;
	...
	public void method2() {
		...
		// NegativeArraySizeException : 배열 할당 과정에서 배열의 크기를 음수로 지정
	...
	public void method3() {
		...
		try {
			...
			int[] arr= new int[size];
			System.out.println("100번 인덱스 값 :"+arr[100]);
		}catch(InputMismatchException e) {//부모에 의해서 실행이 안됨.순서가 몹시 중요
			System.out.println("정수가 아닌 값을 입력하셨습니다.");
			
		}catch(RuntimeException e) {
			System.out.println("예외가 발생되긴 했음..배열의 크기가 잘못 됐던가,부적절한 인덱스가 제시됐던가");//모든 자식 예외 발생시 다 받아서 처리할 수 있음
		} 		
		
		//부모 예외 클래스와 자식 예외 클래스를 가지고 catch 블럭을 기술하고자 할 때는
		//자식 예외 클래스 catch 블럭이 위에 있어야함! (아니면 unreachable catch block 오류 발생)
		// RuntimeException 관련 애들은 UnCheckedException (예외처리구문이 필수는 아니다)
		// if문: 애초에 예외자채가 발생되기 전에 소스코드로 핸들링을 함(예외처리 구문이 아님)
		//try-catch문 : 예외가 "발생했을경우" 처리해주는 구문을 작성해두는것(예외처리 구문)
