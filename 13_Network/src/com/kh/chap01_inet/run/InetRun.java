...
public class InetRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 서버와 클라이언트	
		 * 서버				: 클라이언트(고객)에게 서비스를 제공해주는 프로그램 (클라이언트의 요청을 처리해서 응답해주는) 서빙역활
		 * 클라이언트		: 서버에 요청하는 컴퓨텨 (서비스를 제공 받는 고객)
		 * 
		...
		// InetAddress :  네트워크 정보 (ip주소 관련) 확인 할 수 있는 클래스
		try {
			...
			// 알고 있는 도메인을 통해서 그 서버에 관련된 정보 얻기
		 ...
		 * 이때 데이터를 입출력 하고자 한다면 서버와 클라이언트 간의 스트림(연결통로)가 필요함
		 * 
		 ...
		 *  >> UDP 방식 : 데이터 전송속도가 빠름 신뢰성 없는 데이터 전달 될 수 있음
		 *  
		 * 
		 */
