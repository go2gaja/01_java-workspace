...
public final class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 3. 한 공간에 여러타입의 데이터 저장가능
		 *   
		 *	 *  =========================================================================================
		 * 컬렉션의 장점
		 *  1. 크기 지정 필요 없음 만일 지정했더라 해도 더 많은 데이터들이 들어오면 알아서 사이즈를 늘려줌
		 *     크기의 제약이 없음
		 *  
		 *  2. 중간에 값을 추가한다거나 삭제하는 경우 값을 땡겨주는 작업(알고리즘)을 직접 기술할 필요 없음
		 *     단지 메소드 호출만으로 알아서 내부적으로 진행될꺼임
		 *    
		 *  3. 방대한 데이터들을 단지 보관만 해두고 조회만 할 경우 배열을 많이 사용
		...
		ArrayList list = new ArrayList(3);// 크기 지정 할 수도 있고 안 할 수도 있음 10
		...
		// 5. size() : 해당 리스트의 사이즈를 반환 시켜주는 메소드( 즉 몇개의 데이터가 담겨있는지)
		
		System.out.println("리스트이 사이즈 : "+ list.size());// 배열과 비교해보자면 length 같은 느낌
		...
		System.out.println(((Music)list.get(1)).getTitle());// Object 타입이기 때문에 강제 형변환 해줘야 된다
		
		// 7. subList(int index1, int index2) : List >> 추출해서 새로운 List 반환 : substring 같은것 
