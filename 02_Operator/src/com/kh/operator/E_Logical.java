...
public class E_Logical {
	/*
	 * 논리 연산자(이항 연산자)
	 * 두 개의 논리값을(true,false) 연산해주는 연산자
	 * 논리 연산한 결과마저도 논리값임
	 * 
	 * 논리값 && 논리값 : 왼쪽과 오른쪽 둘다 true일 경우 결과값은 true
	 * 논리값 || 논리값 : 왼쪽과 오른쪽 둘중 하나라도 true 일 경우 결과값은 true
	 * 
	 */
	...
	public void method1 ( ) {
		...
		boolean result = (num >= 1) && (num <=100);//num 값이 1 이상 "이고" "그리고" num 값이 100 이하일 경우
		
		System.out.println("사용자가 입력한 값이 1이상 100이하 입니까? :"+result);
		
		// &&: 두개의 조건 모두가 true야지 &&연산의 결과값이 최종적으로 true
		...
		public void method2 () {
			...
			public void metho3 () {
				...
				// || : 두개의 조건중 하나라도 true 가 있다면 ++ 연산의 결과값은 true
			...
			/*
			 *  && : 두개의 조건이 모두 true 야지 결과값이 true임 ( AND ==~이고 , 그리고, ~이면서, 뿐만아니라)
			 *  
			 *  ture && true => true
			 *  true && false => false
			 *  false && true => false
			 *  false && false => false
			 *  
			 *  && 연산자를 기준으로 앞의 결과가 false 일경우 뒤의 조건을 굳이 실행하지 않는다
			 *  
			 *  
			 *  || : 두개의 조건중 하나라도 true 라면 결과값이 true 임 (OR ==도는, ~이거나) 
			 *  
			 *  true || true => true
			 *  true || false => true
			 *  false || true => true
			 *  false || false =. false
			 *  
			 *  ||연산자 기준으로 앞의 결과가 true 일 경우 뒤의 조건 굳이 실행하지 않는다
			 *  
			 *  
			 *  
			 */
			...
			public void method5 ( ) {
				...
				boolean result2 = true || (++num2 > 0);//Dead Code : 절대로 실행될 일 없는 코드
